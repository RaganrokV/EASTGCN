...
import time
from torch_geometric_temporal.nn.recurrent import GConvGRU,TGCN2,GCLSTM
...
class ASTCG_net(torch.nn.Module):
    def __init__(self, in_channels, embed_channels, out_channels,adj_matrix):
        ...
        self.Att=nn.MultiheadAttention(in_channels, 4, batch_first=True)
...
#%% NO ATT
class ASTCG_no_ATT(torch.nn.Module):
    def __init__(self, in_channels, embed_channels, out_channels,adj_matrix):
        super(ASTCG_no_ATT, self).__init__()
        """:param"""
        self.ADJ=torch.tensor(adj_matrix).to(device)
        self.embed_size=12
        self.weight_inflation=1
    ...
    def forward(self, x, edge_index, edge_weight ):
        ...
        Spatial_info = self.Spatial_cell(x, edge_index, edge_weight) #Spatial_info(B, N_nodes, F_out)
...
#%% no spatial
class ASTCG_no_spatial(torch.nn.Module):
    def __init__(self, in_channels, embed_channels, out_channels,adj_matrix):
        super(ASTCG_no_spatial, self).__init__()
        """:param"""
        self.ADJ=torch.tensor(adj_matrix).to(device)
        self.embed_size=12
        self.weight_inflation=25
        ...
        self.linear=torch.nn.Linear(embed_channels*3, out_channels)
        # self.embedding = torch.nn.Linear(embed_channels * 3, self.embed_size)
        self.embedding = torch.nn.Linear(1*embed_channels , self.embed_size)
    ...
    def forward(self, x, edge_index, edge_weight ):
        ...
        # # Spatial_info = F.relu(Spatial_info)
        #
        # Replacement_info=torch.zeros_like(Spatial_info)
        ...
        # CAT = torch.cat((Temporal_info, ), dim=2)

        embeding=self.embedding(Temporal_info)
        ...
        return Fusion
#%% no seq
class ASTCG_no_seq(torch.nn.Module):
    def __init__(self, in_channels, embed_channels, out_channels,adj_matrix):
        super(ASTCG_no_seq, self).__init__()
        """:param"""
        self.ADJ=torch.tensor(adj_matrix).to(device)
        self.embed_size=12
        self.weight_inflation=25
        ...
        self.linear=torch.nn.Linear(embed_channels*2, out_channels)
    ...
    def forward(self, x, edge_index, edge_weight ):
        ...
        CAT = torch.cat((Spatial_info, Temporal_info), dim=2)

        CAT = F.relu(CAT)
        Fusion=self.linear(CAT)
...
trainX, trainY, valX, valY, testX, testY = divide_data(data=Norm_TS, train_size=train_size,val_size=0,
...
# net = ASTCG_no_ATT(in_channels=seq_len, embed_channels=32,
#                 out_channels=pre_len,adj_matrix=adj_matrix.values).to(device)
# net = ASTCG_no_spatial(in_channels=seq_len, embed_channels=32,
#                 out_channels=pre_len,adj_matrix=adj_matrix.values).to(device)
# net = ASTCG_no_seq(in_channels=seq_len, embed_channels=32,
#                 out_channels=pre_len,adj_matrix=adj_matrix.values).to(device)
...
print(f"神经网络参数量为: {num_parameters} 个")